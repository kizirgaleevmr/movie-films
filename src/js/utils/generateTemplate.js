 ...
 */
const createCardTemplate = (content, useSlider) => {
  return `
    <div class="${useSlider ? "swiper-slide card" : "card"}" data-id="${content?.id}">
      <img src="https://image.tmdb.org/t/p/${useSlider ? "w500" : "w300"}${content?.poster_path}" alt="${content?.title}" />
      ...
      <div class="${useSlider ? "slider-descr" : "card-descr"}">
        <h3 class="${useSlider ? "slider-title" : "card-title"}">${content?.title || content?.name}</h3>
